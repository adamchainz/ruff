...
impl<'src> Parser<'src> {
    ...
    fn parse_slice(&mut self) -> Expr {
        ...
        // test_ok parenthesized_walrus_index_before_310
        // # parse_options: {"target-version": "3.9"}
        // lst[(x:=1)]

        // test_ok unparenthesized_walrus_index_after_310
        // # parse_options: {"target-version": "3.10"}
        // lst[x:=1]

        // test_err unparenthesized_walrus_index_before_310
        // # parse_options: {"target-version": "3.9"}
        // lst[x:=1]
    ...
    fn parse_set_or_dict_like_expression(&mut self) -> Expr {
        ...
        match self.current_token_kind() {
            TokenKind::Async | TokenKind::For => {
                ...
                // test_ok parenthesized_walrus_before_310
                // # parse_options: {"target-version": "3.9"}
                // {(x := 1), 2, 3}
                // {(last := x) for x in range(3)}

                // test_ok unparenthesized_walrus_after_310
                // # parse_options: {"target-version": "3.10"}
                // {x := 1, 2, 3}
                // {last := x for x in range(3)}

                // test_err unparenthesized_walrus_set_comp_before_310
                // # parse_options: {"target-version": "3.9"}
                // {last := x for x in range(3)}
                if self.options.target_version < PythonVersion::PY310
            ...
            _ => {
                // test_err unparenthesized_walrus_set_literal_before_310
                // # parse_options: {"target-version": "3.9"}
                // {x := 1, 2, 3}
                // {1, x := 2, 3}
                // {1, 2, x := 3}
